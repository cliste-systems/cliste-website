...
/**
 * ColorBends: animated WebGL gradient bands.
 * Adapted from React Bits (https://reactbits.dev/backgrounds/color-bends,
...
export function ColorBends({
  ...
  /** Stop the RAF loop when offscreen. Continuous WebGL was janking page scroll. */
  ...
  useEffect(() => {
    ...
    const arr = (colors || [])
      ...
      .slice(0, MAX_COLORS)
      .map(toVec3);
    const slot = material.uniforms.uColors.value as THREE.Vector3[];
    for (let i = 0; i < MAX_COLORS; i++) {
      if (i < arr.length) slot[i].copy(arr[i]);
      else slot[i].set(0, 0, 0);
    }
