import { RETAIL_SETUP } from "@/lib/retail-copy";
import {
...
export function RetailSetup() {
  return (
    <section
      aria-labelledby="retail-setup-heading"
    ...
    >
      <div className={`relative min-w-0 ${RETAIL_SECTION_FRAME}`}>
        <div className={RETAIL_SECTION_INTRO_GRID}>
          <h2 id="retail-setup-heading" className={RETAIL_SECTION_INTRO_TITLE}>
            <span className="text-slate-900">{RETAIL_SETUP.titleDark}</span>
            <span className="bg-clip-text pb-0.5 text-transparent bg-gradient-to-r from-slate-600 via-slate-500 to-slate-400">
              {RETAIL_SETUP.titleGradient}
            </span>
          </h2>
          <p className={SECTION_INTRO_DESC}>{RETAIL_SETUP.intro}</p>
        </div>
